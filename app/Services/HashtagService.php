...
class HashtagService
{
    ...
    {
        return Cache::remember('services:hashtag:by_id:' . $id, now()->addMinutes(60), function() use($id) {
            $tag = Hashtag::find($id);
            if(!$tag) {
                return [];
            }
    ...
    {
        return Cache::remember('services:hashtag:total-count:by_id:' . $id, now()->addMinutes(60), function() use($id) {
            $tag = Hashtag::find($id);
